...
![The Bias-Variance Tradeoff](5.Classification_files/fig3.10.PNG)
...
![](5.Classification_files/fig3.12.PNG)
...
#| code-fold: false
library(mlr)
makeResampleDesc(method = "Holdout", split = 2/3, stratify = TRUE)-> holdout
```
...
```{r}
#| code-fold: false
resample(learner = knn,
         task = diabetesTask,
         resampling = holdout,
         measures = list(mmce, acc)) -> holdoutCV
holdoutCV
```

- The *mean misclassification error* and the *accuracy* values are `r holdoutCV$aggr` respectively.

---

- We can notice two things here:
  1. The accuracy of the model as estimated by the **Holdout CV** is less than when we evaluated its performance on the data we used to train the full model. This exemplifies the point earlier that models will perform better on the data
...
`r performance(pred = knnPred, measures = list(mmce, acc))`
:::
...
```{r}
#| code-fold: false
calculateConfusionMatrix(pred = holdoutCV$pred, relative = TRUE)
```
